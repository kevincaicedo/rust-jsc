...
impl JSObject {
    ...
    ///
    /// # Safety
    /// This function is unsafe because `JSObject` is a handle that can be cloned.
    /// If other references to the private data exist (e.g. obtained via `get_private_data`
    /// from a cloned handle), calling this method will free the memory they point to,
    /// causing Undefined Behavior (Use-After-Free).
    ///
    /// The caller must ensure that no other references to the private data are in use.
    ///
    /// # Example
    /// ```rust,ignore
    /// use rust_jsc::*;
    ...
    /// unsafe { object.set_private_data(42i32) }.unwrap();
    ///
    /// let private_data = unsafe { object.take_private_data::<i32>() }.unwrap();
    /// assert_eq!(private_data, 42);
...
mod tests {
    ...
    fn test_object_take_private_data_type_safe() {
        ...
        let taken = unsafe { object.take_private_data::<i32>() }.unwrap();
        assert_eq!(taken, 99);
        assert!(object.get_private_data::<i32>().is_none());
    }
    ...
    fn test_object_private_data_uaf_scenario() {
        ...
        // 2. Take ownership from alias
        // This frees the memory while data_ref is still active
        let taken = unsafe { object_alias.take_private_data::<String>() };
        assert_eq!(taken.unwrap(), "alive");

        // 3. data_ref is now dangling!
        // Accessing it here would be UAF.
        // println!("Dangling ref: {}", data_ref); // UNCOMMENTING THIS CAUSES UB/CRASH

        // Verify the data is logically gone
